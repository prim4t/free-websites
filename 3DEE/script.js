...
var setcolor = "rgb(28, 28, 28)";

scene.background = new THREE.Color(setcolor)
scene.fog = new THREE.Fog(setcolor, 2.5, 3.4);
...
function generateParticle(num, amp = 2) {
  var gmaterial = new THREE.MeshPhysicalMaterial({color:"rgb(250, 203, 5)", side:THREE.DoubleSide});

  var gparticular = new THREE.CircleGeometry(0.2,15);

  for (var i = 1; i < num; i++) {
    var pscale = 0.011+Math.abs(mathRandom(0.03));
...
function init() {
  for (var i = 0; i<30; i++) {
    ...
    var material = new THREE.MeshStandardMaterial({shading:THREE.FlatShading, color:"rgb(250, 203, 5)", transparent:false, opacity:1, wireframe:false});
...
//------------------------------------------------------------- SCENE
var ambientLight = new THREE.AmbientLight("rgb(250, 203, 5)", 0.1);
//scene.add(ambientLight);

var light = new THREE.SpotLight("rgb(250, 203, 5)", 3);
...
var lightBack = new THREE.PointLight("rgb(250, 203, 5)", 1);
...
var rectSize = 2;
var intensity = 10;
var rectLight = new THREE.RectAreaLight( "rgb(250, 203, 5)", intensity,  rectSize, rectSize );
...
function animate() {
  ...
  //---
  particularGruop.rotation.x += 0.005;
  //---
